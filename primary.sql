...
The Business Question: How does the variance between the estimated delivery date and
the actual delivery date impact customer review scores?
Which states or regions suffer from the worst delays?

Why it matters: Late deliveries hurt brand loyalty.
Finding bottlenecks helps the business negotiate better shipping terms or
choose better regional carriers.

Resume Skills to Highlight: Date-time manipulation (TIMESTAMPDIFF or Pandas to_datetime),
handling geospatial/ZIP data, and 
calculating correlation coefficients between delivery lag and review scores.
-------------------------------------------------------------------------------------------------
2. Financial Metrics: High Freight-to-Price Ratios
The Business Question: Which product categories have a freight cost that accounts
for more than 20% of the total item price?
Does high freight cost correlate with higher order abandonment or
lower customer satisfaction?

Why it matters: Brazil is geographically massive,
and shipping heavy or cheap items long distances can make them unprofitable.
This analysis helps identify products that need regional warehousing or
localized suppliers.

Resume Skills to Highlight: Aggregate formulas, percentage-of-total calculations,
and creating scatter plots with Seaborn to map Price vs. Freight.
---------------------------------------------------------------------------------------------------
3. Customer Retention & Repeat Purchasing Behavior
The Business Question: What percent of customers are one-time buyers versus repeat buyers?
For repeat buyers, what is the average time window between their first and second purchase?

Why it matters: E-commerce businesses thrive on customer lifetime value (LTV).
If 95%+ of customers buy only once, the business is spending too much on acquisition and
not enough on retention.

Resume Skills to Highlight: Self-joins, CTEs to isolate the "first purchase date," 
and Window Functions (LEAD or LAG) to find the difference between consecutive order dates.
------------------------------------------------------------------------------------------------------
4. Seller Performance & "Super-Seller" Concentration
The Business Question: What percentage of total revenue is generated by the top 5% 
of sellers? Are these top sellers dominating specific product categories, 
or are they spread out?

Why it matters: Over-reliance on a few "super-sellers" introduces platform risk. 
If a single seller generating 10% of your revenue leaves, 
the platform takes a massive hit.

Resume Skills to Highlight: Cumulative percentage calculations using 
DENSE_RANK() or SUM() OVER(ORDER BY ...) windows, and
a Pareto analysis (80/20 rule) chart.
--------------------------------------------------------------------------------------------------------
5. Payment Methods & Order Value
The Business Question: Do customers who pay with credit cards split into 
multiple installments (payment_installments) buy more expensive items or 
have higher overall order values than those who pay via Boleto (cash voucher) or 
debit card?

Why it matters: Understanding installment behavior allows marketing teams to
run targeted promotions
(e.g., "Interest-free up to 6 installments for high-value electronics").

Resume Skills to Highlight: Categorical grouping,
handling multiple payment rows per order (since an order can be split across payment types),
